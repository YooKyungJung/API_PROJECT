...
function find2(){//find2 버튼을 눌렀을 때(onClick)
	...
	daum.maps.event.addListener(map, 'dragend', function() {
		handleRefresh2();//지도의 중심이 이동될때도 마커를 다시 표시
	});
	
	handleRefresh2();//검색버튼을 클릭할 때 마커 표시
}//find2
...
function handleRefresh2() {
	...
	{
	var j = i + 999;
    var url="http://openapi.seoul.go.kr:8088/4a5243534579756736356f4a4c6d41/json/MgisSummerStreet/"+i+"/"+j;
    $.getJSON(url, updateLibrary2);
...
function updateLibrary2(librarys) {//16번 호출
   var librarys = librarys.MgisSummerStreet.row;
   var addr = "";
   ...
   for (var i = 0; i < librarys.length; i++) {
      var lib = librarys[i];
      //console.log(lib.COT_COORD_X);
      var imageSrc = "tree.png",
      ...
      var loc = {//open API의 값들 위도와 경도
            latitude : lib.COT_COORD_X,
			longitude: lib.COT_COORD_Y
      };
	  var km = computeDistance2 (position, loc); //거리 계산, position 지도의 중심좌표이고 loc는 각 주차장 좌표
	// if(addr != lib.COT_ADDR_FULL_OLD && km <= 10){//주소가 중복되지 않고, 거리가 2km이내의 것들을 가져온다.
		 addr = lib.COT_ADDR_FULL_OLD;
		 //addr = "서울특별시 ".concat(lib.COT_ADDR_FULL_OLD);
		 //console.log(addr);
		  addMarker2(imageSrc, imageSize, imageOption, lib.COT_COORD_X, lib.COT_COORD_Y, lib.COT_CONTS_NAME,lib.COT_ADDR_FULL_OLD,lib.COT_VALUE_02, lib.COT_VALUE_03);
	//}
   ...
   var librarysDiv = document.getElementById("librarys");
	...
	for(var i=0; i<librarys.length; i++){
		...
		div.innerHTML="";
		//console.log(lib.COT_GU_NAME);
		if (lib.COT_GU_NAME == gus) {
			div.innerHTML = "[" + lib.COT_NAME_04 + "]" + lib.COT_CONTS_NAME;
			
			div.innerHTML += "<input type=button value=위치" + " onclick=\"window.open('http://www.google.co.kr/maps/search/" + lib.COT_CONTS_NAME + " ')\"/>"; 
			
			if (lib.COT_ADDR_FULL_NEW != "") {
				div.innerHTML += "<br>" + "주소: " + lib.COT_ADDR_FULL_NEW + "(x:"
						+ lib.COT_COORD_Y + "&nbsp;,&nbsp;y:" + lib.COT_COORD_X + ")";
			}
			
			if (lib.COT_VALUE_01 != "") {
				div.innerHTML += "<br>" + "소개: " + lib.COT_VALUE_01;
			}
			
			if (lib.COT_VALUE_02 != "") {
				div.innerHTML += "<br>주요 수종: " + lib.COT_VALUE_02;
			}
			
			if (lib.COT_VALUE_03 != "") {
				div.innerHTML += "<br>규모: " + lib.COT_VALUE_03;
			}
			
			if (lib.COT_VALUE_05 != "") {
				div.innerHTML += "<br>문의처: " + lib.COT_VALUE_05;
			}
...
function addBound2(){
	...
	var bound = new daum.maps.Circle({
	   ...
	   strokeColor: '#3b8686', // 선의 색깔입니다
	   strokeOpacity: 0.5, // 선의 불투명도 입니다 1에서 0 사이의 값이며 0에 가까울수록 투명합니다
	   strokeStyle: 'solid', // 선의 스타일 입니다
	   fillColor: '#3b8686', // 채우기 색깔입니다
...
//마커추가
function addMarker2(imageSrc, imageSize, imageOption, latitude, longitude, name, address,tel, closeday) {
	//console.log(latitude+"드러옴"+longitude);
	//이미지 마커 맵에 추가
...
//거리계산. 준비된 코드
function computeDistance2  (startCoords,destCoords){
    
    var startLatRads = degreesToRadians2(startCoords.latitude);
    var startLongRads =degreesToRadians2(startCoords.longitude);
    var destLatRads = degreesToRadians2(destCoords.latitude);
    var destLongRads = degreesToRadians2(destCoords.longitude);
...
function nearlibrarylocation2(){
	...
	navigator.geolocation.getCurrentPosition(function(position) {
	    ...
	    var locPosition = new daum.maps.LatLng(37.466350, 126.932893), // 마커가 표시될 위치를 geolocation으로 얻어온 좌표로 생성합니다
	        message = '<div style="padding:5px;">당신의 위치입니다.</div>'; // 인포윈도우에 표시될 내용입니다
	...
	var locPosition = new daum.maps.LatLng(37.466350, 126.932893),    
	    message = 'geolocation을 사용할수 없어요..'
