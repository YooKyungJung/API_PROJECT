...
function handleRefresh() {
	...
	{
	var j = i + 999;
    var url="http://openapi.seoul.go.kr:8088/4a5243534579756736356f4a4c6d41/json/MgisSpringStreet/"+i+"/"+j;
...
function updateLibrary(librarys) {//16번 호출
   var librarys = librarys.MgisSpringStreet.row;
   var addr = "";
   ...
   for (var i = 0; i < librarys.length; i++) {
	...
	// if(addr != lib.COT_ADDR_FULL_OLD && km <= 10){//주소가 중복되지 않고, 거리가 2km이내의 것들을 가져온다.
		 ...
		 //console.log(addr);
		  addMarker(imageSrc, imageSize, imageOption, lib.COT_COORD_X, lib.COT_COORD_Y, lib.COT_CONTS_NAME,lib.COT_ADDR_FULL_OLD,lib.COT_VALUE_02, lib.COT_VALUE_03);
	//}
   ...
   var librarysDiv = document.getElementById("librarys");
	...
	for(var i=0; i<librarys.length; i++){
		...
		if (lib.COT_GU_NAME == gus) {
			div.innerHTML = "[" + lib.COT_VALUE_05 + "]" + lib.COT_CONTS_NAME;
			...
			if (lib.COT_VALUE_02 != "") {
				div.innerHTML += "<br>주요봄꽃: " + lib.COT_VALUE_02;
			}
			
			if (lib.COT_VALUE_03 != "") {
				div.innerHTML += "<br>예상개화시기: " + lib.COT_VALUE_03;
			}
			
			if (lib.COT_VALUE_04 != "") {
				div.innerHTML += "<br>규모: " + lib.COT_VALUE_04;
			}
			
			if (lib.COT_VALUE_06 != "") {
				div.innerHTML += "<br>문의처: " + lib.COT_VALUE_06;
			}
...
function addBound(){
	...
	var bound = new daum.maps.Circle({
	   ...
	   strokeColor: '#fe4365', // 선의 색깔입니다
	   strokeOpacity: 0.5, // 선의 불투명도 입니다 1에서 0 사이의 값이며 0에 가까울수록 투명합니다
	   strokeStyle: 'solid', // 선의 스타일 입니다
	   fillColor: '#fe4365', // 채우기 색깔입니다
...
function addMarker(imageSrc, imageSize, imageOption, latitude, longitude, name, address,tel, closeday) {
	...
	var content =  "<div style='width:100%; height:100%; padding:5px; font-size:0.8em;'>"+'<b>'+ name +'</b>'+'<br>'+"주소 : " +address+'<br>'+"주요 봄꽃 : "+tel+'<br>'+"개화시기 : " +closeday+" </div>";
